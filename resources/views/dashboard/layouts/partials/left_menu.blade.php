...
<aside class="app-sidebar">
    <ul class="app-menu">
        ...
        @foreach (session('menus')->where('parent_id', 0)->where('action_menu', 0)->sortBy('order_serial') as
        $parentMenu)

        @if (!count(session('menus')->where('action_menu', 0)->where('parent_id',
        $parentMenu->id)->sortBy('order_serial')))

        @can($parentMenu->name)
        <li>
            <a class="app-menu__item @if (Route::currentRouteName() == $parentMenu->url ) active @endif"
                href="{{ route($parentMenu->url) }}">
                <i class="app-menu__icon {{ $parentMenu->icon }}" style="color:{{ $parentMenu->icon_color }};"></i>
        ...
        <li class="treeview {{ $openSubMenu }}">
            <a class="app-menu__item" href="#" data-toggle="treeview">
                <i class="app-menu__icon {{ $parentMenu->icon }}" style="color:{{ $parentMenu->icon_color }};">
                </i>
            ...
            <ul class="treeview-menu">
                ...
                <li>
                    <a class="treeview-item @if (Route::currentRouteName()== $ChildMenu->url ) active @endif"
                        ...
                        <i class="icon {{ $ChildMenu->icon }}" style="color:{{ $ChildMenu->icon_color }};"></i>
                        {{ $ChildMenu->name }}
                    </a>
                </li>
                @endcan
