...
@section('content')
    <form method="POST" action="{{ route('cashrequisition.store') }}">
        @csrf
        <div class="tile">
            <div class="tile-body">
                <div class="">
            ...
            <div class="tile-body">
                ...
                <div class="row material_table">
                    <div class="col-md-12" style="overflow-y: auto; max-height: 600px;">
                        <table class="table gridTable mobile-changes d-none">
                            <thead>
                                <tr>
                            ...
                            <tfoot>
                                <tr>
                                    <th colspan="6" class="px-1 py-1">
                                        <p class='mb-0 mt-2 text-right'>Total Requisition</p>
                                    </th>
                                    <th class="py-2 py-1">
                                        <input class="text-center total_amount form-control text-right" type="number"
                                            step="any" name="total_amount" value="0" readonly>
                                    </th>
                                    <th></th>
                                    <th align="center" class="py-0">
...
@section('custom-script')
    ...
    <script>
        ...
        function updatePending(id) {
            // update approved due
            let approvedAmount = $('.approved_amount_' + id).val();
            let issuedAmount = $('.issued_' + id).val();

            let balance = approvedAmount - issuedAmount;

            $('.approved_due_' + id).val(balance);

            // set max limit in current requisition ( budget amount - approved amount)
            let estimatedAmount = $('.estimated_' + id).val();
            let approvedDue = $('.approved_due_' + id).val();

            let maxLimit = estimatedAmount - approvedDue;

            $('.amount_' + id).attr('max', maxLimit);
        }
    ...
    <script type="text/javascript">
        function addItem() {
            ...
            $(".gridTable tbody").append('<tr id="itemRow_' + total + '">' +
                ...
                ')" class="form-control chosen-select itemList itemList_' + total + '" required="required">' +
                '</select>' +
                '</td>' +
                '<td>' +
                ...
                '</select>' +
                '</td>' +
                '<td><input class="text-center form-control estimated_' + total +
                '" type="number" step="any" name="estimated[]" value="0" required readonly></td>' +
                '<td><input class="text-center form-control approved_amount_' + total +
                '" type="number" step="any" name="approved_amount[]" value="0" required readonly></td>' +
                '<td><input class="text-center form-control issued_' + total +
                '" type="number" step="any" name="issued[]" value="0" required ' + rate_attr + ' readonly></td>' +
                '<td><input class="text-center form-control approved_due_' + total +
                '" type="number" step="any" name="approved_due[]" value="0" required ' + rate_attr + ' readonly></td>' +
                '<td><input class="text-center form-control amount amount_' + total +
                ...
                '<td align="center"><button class="btn btn-success" onclick="addItem()"> <i class="fa fa-plus" aria-hidden="true" style="font-size: 20px"></i> </button> <button class="btn btn-danger item_remove" onclick="itemRemove(' +
                total + ')"><i class="fa fa-trash" style="font-size: 20px"></i> </button></td>' +
                '</tr>');
            $('.row_count').val(total);
