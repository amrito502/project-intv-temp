...
@section('content')
    ...
    <form action="{{ route('cashrequisition.update', $data->cashrequisition->id) }}" method="POST">
        @csrf
        @method('patch')
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control"
                                    value="{{ $data->cashrequisition?->tower?->name }}" readonly>
                                    ...
                                    <tbody id="tbody">
                                        @foreach ($data->cashrequisition->items as $item)
                                            <tr id="itemRow_{{ $item->id + 999 }}">
                                                    ...
                                                    <select name="vendor[]"
                                                        ...
                                                        @foreach ($data->vendors as $vendor)
                                                            <option value="{{ $vendor->id }}"
                                                                @if ($vendor->id == $item->vendor_id) selected @endif>
                                                                {{ $vendor->name }}</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                    ...
                                                    <select name="budget_head[]"
                                                        ...
                                                        @foreach ($data->budgetHeads as $budgetHead)
                                                            <option value="{{ $budgetHead->id }}"
                                                                @if ($budgetHead->id == $item->budget_head_id) selected @endif>
                                                ...
                                                <td>
                                                    <input
                                                        class="text-center form-control estimated_{{ $item->id + 999 }}"
                                                        type="number" step="any" name="estimated[]" value=""
                                                        required readonly>
                                                </td>
                                                <td>
                                                    <input
                                                        class="text-center form-control approved_amount_{{ $item->id + 999 }}"
                                                        type="number" step="any" name="approved_amount[]"
                                                        value="{{ $item->approved_amount_log }}" required readonly>
                                                </td>
                                                <td>
                                                    <input class="text-center form-control issued_{{ $item->id + 999 }}"
                                                        type="number" step="any" name="issued[]"
                                                        value="{{ $item->issued_amount_log }}" required readonly>
                                                </td>
                                                <td>
                                                    <input
                                                        class="text-center form-control approved_due_{{ $item->id + 999 }}"
                                                        type="number" step="any" name="approved_due[]"
                                                        value="{{ $item->approved_due_log }}" readonly>
                                                </td>
                                                <td>
                                                    <input
                                                        class="text-center form-control amount amount_{{ $item->id + 999 }}"
                                                        type="number" step="any" name="amount[]"
                                                        value="{{ $item->requisition_amount }}" onkeyup="row_sum()"
                                                        required>
                                                </td>
                                                <td>
                                                    <input class="form-control remarks remarks_{{ $item->id + 999 }}"
                                                        type="text" name="item_remarks[]"
                                                        value="{{ $item->remarks }}">
                                                </td>
                                                <td align="center">
                                    ...
                                    <tfoot>
                                        <tr>
                                            <th colspan="6" class="px-1 py-1">
                                                <p class='text-right mb-0 mt-2'>Total Requisition</p>
                                            </th>
                                            <th class="py-2 py-1">
                                                <input class="text-center total_amount form-control text-right"
                                                    type="number" step="any" name="total_amount" value="0"
                                                    readonly>
                                            </th>
                                            <th></th>
                                            <td align="center" class="py-0">
                                                <input type="hidden" class="row_count" value="1">
                                                <span class="btn btn-lg btn-block btn-success add_item">
    ...
    <script type="text/javascript">
        function addItem() {
            ...
            $(".gridTable tbody").append('<tr id="itemRow_' + total + '">' +
                ...
                '</select>' +
                '</td>' +
                '<td><input style="text-align: right;" class="form-control estimated_' + total +
                '" type="number" step="any" name="estimated[]" value="0" required readonly></td>' +
                '<td><input style="text-align: right;" class="form-control approved_amount_' + total +
                '" type="number" step="any" name="approved_amount[]" value="0" required readonly></td>' +
                '<td><input style="text-align: right;" class="form-control issued_' + total +
                '" type="number" step="any" name="issued[]" value="0" required readonly></td>' +
                '<td><input style="text-align: right;" class="form-control approved_due_' + total +
                '" type="number" step="any" name="approved_due[]" value="0" required ' + rate_attr + ' readonly></td>' +
                '<td><input style="text-align: right;" class="form-control amount amount_' + total +
                '" type="number" step="any" name="amount[]" value="0" onkeyup="row_sum()" required></td>' +
                '<td><input class="form-control remarks remarks_' + total +
