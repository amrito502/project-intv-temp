    ...
    public static function dataTableAjax()
    {
        ...
        return DataTables::eloquent($category)
            ...
            ->addColumn('status_ui', function ($row) {
                ...
                if (Auth::user()->can('status category')) {
                    return $slide_button;
                }
                return "";
            })
            ...
            ->addColumn('action', function ($row) {
                ...
                if (Auth::user()->can('view category')) {
                    $btn = $btn . '<a href="' . route('category.show', $row->id) . '" class="edit btn btn-info btn-sm m-1"><i class="fa fa-eye" aria-hidden="true"></i>
 View</a> &nbsp;';
                }

                if (Auth::user()->can('edit category')) {
                    $btn = $btn . '<a href="' . route('category.edit', $row->id) . '" class="edit btn btn-primary btn-sm"><i class="fa fa-pencil" aria-hidden="true"></i>
 Edit</a> &nbsp;';
                }
